  ...
  const handleSubmit = async (e) => {
    ...
    if (imageFile) {
      try {
        const imageRef = ref(
          storage,
          `recipe_images/${Date.now()}-${imageFile.name}`,
        );
        ...
        if (step.imageFile) {
          try {
            const stepImageRef = ref(
              storage,
              `step_images/${Date.now()}-${step.imageFile.name}`,
            );
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              ...
              <p className="text-gray-500 text-sm">
                {imageFile
                  ? imageFile.name
                  : "Haz clic para subir una imagen de tu receta"}
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            {steps.map((step, index) => (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleStepChange(index, "flameLevel", e.target.value)
                      }
                ...
                <div>
                  <label className="block text-gray-700 text-sm font-medium mb-1">
                    Ingrediente principal del paso
                  </label>
                  <input
                    type="text"
                    value={step.ingredient}
                    onChange={(e) =>
                      handleStepChange(index, "ingredient", e.target.value)
                    }
                    className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-orange-400 focus:outline-none"
