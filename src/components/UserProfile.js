    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        ...
        try {
          ...
          console.error("Error fetching profile data or favorites:", err);
          showNotification("No se pudo cargar tu perfil. Inténtalo de nuevo.", "error");
        } finally {
          setLoading(false);
        }
      } else {
    ...
    <>
      <div className="min-h-screen bg-gradient-to-br from-yellow-400 via-orange-400 to-pink-500">
        ...
        <div className="p-6 max-w-4xl mx-auto">
          ...
          {/* Tabs for Profile and My Favorites */}
          ...
          {activeTab === "profile" && (
            <AnimatePresence>
              ...
              <motion.div
                ...
                exit={{ opacity: 0, y: -20 }}
              >
                {/* Puedes mover el contenido principal del perfil aquí si lo deseas */}
                {/* Por ahora, dejaremos el mensaje que tenías */}
              </motion.div>
            </AnimatePresence>
          )}
        </div>
