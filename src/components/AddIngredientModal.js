...
const AddIngredientModal = ({ isOpen, onClose, onIngredientAdded, currentIngredient = null }) => {
  const [name, setName] = useState(currentIngredient ? currentIngredient.name : '');
  const [price, setPrice] = useState(currentIngredient ? currentIngredient.price : '');
  const [imageFile, setImageFile] = useState(null);
  const [imageUrl, setImageUrl] = useState(currentIngredient ? currentIngredient.image : '');
  ...
  const handleSubmit = async (e) => {
    ...
    if (imageFile) {
      try {
        const imageRef = ref(storage, `ingredient_images/${Date.now()}-${imageFile.name}`);
    ...
    } else if (!currentIngredient) { // If it's a new ingredient and no image is provided
    ...
    try {
      ...
      }, { merge: true }); // Use merge to add to array without overwriting
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      ...
      >
        ...
        <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center">
          {currentIngredient ? 'Actualizar Ingrediente' : 'Agregar Nuevo Ingrediente'}
        </h2>

        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-gray-700 font-semibold mb-2">Nombre del Ingrediente</label>
            <input
              type="text"
              value={name}
          ...
          <div>
            <label className="block text-gray-700 font-semibold mb-2">Imagen del Ingrediente</label>
          ...
          <motion.button
            ...
            className={`w-full py-3 rounded-2xl font-bold text-lg flex items-center justify-center gap-2 ${
              loading ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-gradient-to-r from-orange-500 to-pink-500 text-white hover:from-orange-600 hover:to-pink-600'
            }`}
            whileHover={loading ? {} : { scale: 1.02 }}
            whileTap={loading ? {} : { scale: 0.98 }}
            disabled={loading}
          >
            {loading ? 'Guardando...' : (currentIngredient ? 'Actualizar Ingrediente' : 'Agregar Ingrediente')}
